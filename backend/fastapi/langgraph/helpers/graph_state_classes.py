...
class BusinessOnlyState(BaseModel):
    ...
    business_contact_info: str = Field(
        description="The email addresses and phone number of the business"
    )
...
class BusinessValidationResult(BaseModel):
    is_valid: bool = Field(
        description="True if the provided output from the llm matches the prompt"
    )
