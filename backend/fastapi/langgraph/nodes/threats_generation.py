...
def generate_threats(state: BusinessState, llm_model_name: str = "llama3.2") -> ThreatItemCollection | BusinessState:
    """
    ...
    try:
        ...
        llm_model_structured_output = llm_model.with_structured_output(ThreatItemCollection)
        ...
        logger.error(f'There was an error while generating threats. Details below: \n{e}')
        return state


def get_validated_threats(state: BusinessState, max_retries: int = 3) -> BusinessState | None:
    """
    ...
    while attempt < max_retries:
        ...
        are_threats_appropriate = validate_generated_output(
            prompt=create_threats_validation_prompt(original_prompt=threat_generator_prompt_message,
