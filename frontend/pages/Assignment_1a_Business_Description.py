...
def assignment_page():
    ...
    st.markdown(
        f'#### The business that was assigned to you is called:\n### "{business_name}"'
    )
    ...
    with st.expander("Expand to see the business assets:", icon="💾", expanded=False):
        ...
        if (
            not assets_obj
            or not isinstance(assets_obj, dict)
            or not assets_obj.get("assets")
            and not st.session_state.assetsGeneratedState
        ):
            with st.spinner("Fetching assets..."):
                updated_state = requests.post(
                    f"{url}/api/assets/generate-assets", json=business_state
                )
                if updated_state.status_code == 200:
                    ...
                    st.error(
                        f"Failed to fetch assets: {updated_state.status_code} - {updated_state.text}"
                    )
                    assets_obj = None
