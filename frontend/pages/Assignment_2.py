...
def assignment_page():
    ...
    business_description = business_state.get(
    ...
    security_assessment_md = requests.get(
        f"{base_url}/api/retrieve-file/retrieve-security-template"
    ).text
    ...
    with st.expander(
        "If you need a reminder, expand to see the business details:",
        icon="➕",
        expanded=False,
    ):
        st.markdown(
            f"*Details below for*: ***{business_name}***<br/>"
        ...
        if (
            ...
            with st.spinner("Fetching hints..."):
                updated_state = requests.post(
                    f"{base_url}/api/threats/generate-threats", json=business_state
                )
                if updated_state.status_code == 200:
                    ...
                    st.error(
                        f"Failed to fetch hints: {updated_state.status_code} - {updated_state.text}"
                    )
                    threats_obj = None

        # Show threats
        if (
            threats_obj is not None
            and isinstance(threats_obj, dict)
            and threats_obj.get("threats")
        ):
            st.markdown(
                "<h3>Consider the following potential threats the business could face</h3><br/>"
                "<p><strong>Can the owner do anything about these threats?</strong></p>",
                unsafe_allow_html=True,
            )
